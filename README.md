...
손금은 사진에서 손바닥 위치를 찾아 주요 선을 자동으로 제안하고, 사용자가 확정합니다. 피부색 분할·내접원·valley 필터를 쓰는 고전 영상처리이며 학습 모델을 쓰지 않아 같은 사진이면 같은 결과를 냅니다. 추출하는 값은 주요 선의 정규화 길이와 곡률뿐이며, 깊이·분기·생명선 의미 판독·의학적 판단은 제공하지 않습니다. 인식은 브라우저에서만 수행되며 사진은 서버로 전송되지 않고 원본 저장 정책은 `never`입니다. 개인 입력의 업무 증거에 높은 가중치를 두고 상징 데이터 가중치는 작게 유지합니다(전체 Trait 가중치의 약 1.4%).
