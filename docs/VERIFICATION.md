...
`src/domain/palm-vision.ts`로 브라우저 전용 고전 영상처리 파이프라인을 추가했다. 의존성은 추가하지 않았다.

- 합성 손바닥 이미지로 감정선·두뇌선·생명선 3개를 모두 추적, 좌표는 [0,1] 범위, 같은 입력에 같은 출력(결정성) 확인.
- 손이 없는 이미지와 너무 작은 손은 선을 만들어내지 않고 이유를 반환.
- 실행 중인 앱에서 파일 입력에 합성 이미지를 주입해 배선 검증: 선 3개 자동 표시, 8점씩 24점 렌더, 6개 값 측정, 선별 신뢰도 표기.
- 자동 표시 후에도 클릭으로 점 추가가 동작해 수동 편집이 유지됨을 확인.
- `head_line_curve`를 바꾸면 적응력 Trait이 바뀌는 것을 확인. 이전에는 매핑만 있고 생성되지 않던 값이다.
- 390×844에서 가로 스크롤 없음.
...
- 손금 자동 인식은 고전 영상처리이다. 피부색 분할로 손을 찾고, 내접원으로 손바닥을 잡고, valley 필터로 주요 선을 추적한다. 학습 모델이 없으므로 결과는 결정적이다. 정확도는 조명·배경·손 각도에 따라 크게 달라지므로 결과는 확정이 아닌 제안으로 제시하고 선별 신뢰도(뚰렷함/보통/흐림)를 표시하며 사용자가 직접 고칠 수 있다. 검증은 합성 손바닥 이미지 기준이며 실제 사람 손에 대한 정확도 측정은 수행하지 않았다. 깊이·분기·의학적 판독은 지원하지 않는다. 모든 처리는 브라우저에서 일어나며 사진은 서버로 전송하지 않는다.
